...
fetch('https://api.openweathermap.org/data/2.5/forecast?q='+newName.value+'&appid=198700fd07cfb27eb62679f850c5d048')
...
.then(data => {
    ...
    //Humidity
    for(i = 0; i<5; i++){
        document.getElementById("day" + (i+1) + "Humidity").innerHTML = "HUMIDITY: " + Number(data.list[i].main.humidity).toFixed(3)+ "%";
        
    }

    //Humidity
    for(i = 0; i<5; i++){
        document.getElementById("day" + (i+1) + "Wind").innerHTML = "Wind Speed: " + Number(data.list[i].wind.speed).toFixed(4)+ "mph";
...
function DefaultScreen(){
    document.getElementById("cityInput").defaultValue = "Bengaluru";
    GetInfo();
}
